...
class OCRProcessor {
    ...
    processConstanciaDocument(text) {
        ...
        // Extract emission date - múltiples patrones incluyendo saltos de línea
        const datePatterns = [
            /(\d{1,2})\s*DE\s*([A-Z]+)\s*DE\s*(\d{4})/i,  // "01 DE AGOSTO DE 2025"
            /(\d{1,2})\s*de\s*([a-z]+)\s*de\s*(\d{4})/i,  // "01 de agosto de 2025"
            /A\s*(\d{1,2})\s*DE\s*([A-Z]+)\s*DE\s*(\d{4})/i, // "A 01 DE AGOSTO DE 2025"
            /(\d{1,2})\s*DE\s*\n\s*([A-Z]+)\s*DE\s*(\d{4})/i, // "01 DE\nAGOSTO DE 2025"
            /A\s*(\d{1,2})\s*DE\s*\n\s*([A-Z]+)\s*DE\s*(\d{4})/i, // "A 01 DE\nAGOSTO DE 2025"
            /(\d{1,2})\s*DE\s*([A-Z]+)\s*\n\s*DE\s*(\d{4})/i, // "01 DE AGOSTO\nDE 2025"
        ];
        ...
        for (const pattern of datePatterns) {
            ...
            if (dateMatch) {
                // Reconstruir la fecha sin saltos de línea
