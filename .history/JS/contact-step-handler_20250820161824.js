...
window.showContactInfo = () => {
    // Cambiar de paso bancario a contacto
    document.getElementById('banking-step').classList.remove('active');
    document.getElementById('contact-step').classList.add('active');
    
    // Actualizar progreso (Paso 4 de 4)
    if (window.formHandler) {
        window.formHandler.updateProgress(100, 'Paso 4 de 4: Datos de Contacto');
        window.formHandler.currentStep = 'contact';
    }
    
    // Inicializar el paso de contacto
    window.contactStepHandler.initializeContactStep();
};

window.proceedToSubmission = () => {
    // Validar formulario de contacto antes de proceder
    ...
    // Llamar a la función de envío del formulario principal
    if (window.formHandler) {
        window.formHandler.submitForm();
    }
};

// Función para actualizar el manejador principal del formulario
window.updateFormHandlerForContact = () => {
    if (window.formHandler) {
        // Actualizar el método showBankingInfo para que vaya al paso de contacto
        const originalShowBankingInfo = window.formHandler.showBankingInfo;
        window.formHandler.showBankingInfo = function() {
            // Ejecutar la lógica original
            originalShowBankingInfo.call(this);
            
            // Cambiar el texto del progreso para ser paso 3 de 4
            this.updateProgress(75, 'Paso 3 de 4: Información Bancaria');
        };

        // Actualizar el método showDocuments para manejar la navegación desde contacto
        const originalShowDocuments = window.formHandler.showDocuments;
        window.formHandler.showDocuments = function() {
            if (this.currentStep === 'contact') {
                // Ir de contacto a bancario
                ...
                // Ejecutar lógica original para otros casos
                originalShowDocuments.call(this);
            }
        };

        // Actualizar el método collectFormData para incluir datos de contacto
        const originalCollectFormData = window.formHandler.collectFormData;
        window.formHandler.collectFormData = function() {
            const formData = originalCollectFormData.call(this);
            
            // Agregar datos de contacto
            if (window.contactStepHandler) {
                formData.contactData = window.contactStepHandler.getContactData();
                formData.contactSummary = window.contactStepHandler.getContactSummary();
            }
            
            return formData;
        };

        // Actualizar el método reset para incluir el paso de contacto
        const originalReset = window.formHandler.reset;
        window.formHandler.reset = function() {
            originalReset.call(this);
            
            // Resetear paso de contacto
            if (window.contactStepHandler) {
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', window.updateFormHandlerForContact);
} else {
    window.updateFormHandlerForContact();
}