...
class OCRProcessor {
    ...
    async processPDF(file, documentType) {
        try {
            ...
            if (documentType === 'constancia') {
                // Para constancia fiscal, procesar TODAS las páginas (la cadena original está en la última)
                numPages = pdf.numPages;
                console.log(`Procesando constancia fiscal: ${numPages} páginas`);
            } else {
    ...
    processConstanciaDocument(text) {
        ...
        // Extract emission date - usar la fecha de la Cadena Original Sello (más confiable)
        ...
        if (cadenaMatch) {
            ...
            console.log('No se encontró la fecha en la Cadena Original, intentando métodos alternativos...');
            console.log('Texto completo para debug:', text.substring(0, 500));
            
            // Fallback a los patrones anteriores si no se encuentra la cadena original
            const datePatterns = [
            ...
            for (const pattern of datePatterns) {
                ...
                if (dateMatch) {
                    ...
                    console.log(`Fecha extraída con patrón alternativo: ${data.emissionDate}`);
                    break;
                }
            }
        }
