...
class CommercialConditionsHandler {
    ...
    validateCiudadesField(event) {
        ...
        if (!value) {
            message = 'Debe especificar al menos una ciudad donde presta servicios';
        } else if (value.length < 3) {
            message = 'Debe especificar al menos una ciudad válida';
        } else {
            // Verificar que tenga al menos una ciudad válida (formato básico)
            const ciudades = value.split(',').map(c => c.trim()).filter(c => c.length > 0);
