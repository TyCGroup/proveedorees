...
class OCRProcessor {
    ...
    processConstanciaDocument(text) {
        ...
        console.log('Buscando fecha en Cadena Original...');
        
        // Buscar patrón más simple: ||YYYY/MM/DD|
        const cadenaOriginalRegex = /\|\|(\d{4})\/(\d{2})\/(\d{2})\|/;
        const cadenaMatch = text.match(cadenaOriginalRegex);
        
        if (cadenaMatch) {
            const year = parseInt(cadenaMatch[1]);
            const month = parseInt(cadenaMatch[2]) - 1; // JavaScript months are 0-based
            const day = parseInt(cadenaMatch[3]);
            data.emissionDate = new Date(year, month, day);
            console.log(`✅ Fecha extraída de Cadena Original: ${day}/${month + 1}/${year}`);
            console.log(`✅ Fecha completa: ${data.emissionDate}`);
        } else {
            console.log('❌ No se encontró la fecha en la Cadena Original');
            
            // Buscar específicamente fechas de AGOSTO 2025 (no fechas viejas como 1990)
            console.log('Buscando fechas de 2025...');
            const datePatterns2025 = [
                /(\d{1,2})\s*DE\s*AGOSTO\s*DE\s*2025/i,
                /(\d{1,2})\s*de\s*agosto\s*de\s*2025/i,
                /A\s*(\d{1,2})\s*DE\s*AGOSTO\s*DE\s*2025/i,
            ];

            let dateMatch = null;
            for (const pattern of datePatterns2025) {
                dateMatch = text.match(pattern);
                if (dateMatch) {
                    const day = parseInt(dateMatch[1]);
                    data.emissionDate = new Date(2025, 7, day); // Agosto = mes 7 (0-indexed)
                    console.log(`✅ Fecha extraída de texto (2025): ${day}/8/2025`);
                    break;
                }
            }
            
            // Solo si no encuentra fechas de 2025, buscar otros patrones
            if (!dateMatch) {
                console.log('Intentando otros patrones de fecha...');
                const fallbackPatterns = [
                    /(\d{1,2})\s*DE\s*([A-Z]+)\s*DE\s*(\d{4})/i,
                    /(\d{1,2})\s*de\s*([a-z]+)\s*de\s*(\d{4})/i,
                    /A\s*(\d{1,2})\s*DE\s*([A-Z]+)\s*DE\s*(\d{4})/i,
                ];

                for (const pattern of fallbackPatterns) {
                    dateMatch = text.match(pattern);
                    if (dateMatch) {
                        const fullDateString = dateMatch[0].replace(/\n\s*/g, ' ');
                        data.emissionDate = this.parseSpanishDate(fullDateString);
                        console.log(`⚠️ Fecha extraída con patrón alternativo: ${data.emissionDate}`);
                        break;
                    }
                }
            }
