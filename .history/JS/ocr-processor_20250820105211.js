...
class OCRProcessor {
    ...
    // Agregar estos métodos a la clase OCRProcessor existente
...
// Update the processBancarioDocument method to store full text
processBancarioDocument(text) {
    const data = {
    ...
    const companyPatterns = [
        ...
        /(?:^|\s)([A-ZÀ-ÿ\s]+CONCEPTO\s+RISOGRAFICO[A-ZÀ-ÿ\s]*)/m,
        /(\d+\s+)?(CONCEPTO\s+RISOGRAFICO)/i,
    ];
