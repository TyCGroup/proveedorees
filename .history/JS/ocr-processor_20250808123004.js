...
class OCRProcessor {
    ...
    processConstanciaDocument(text) {
        ...
        // Extract company name (Denominación/Razón Social) - múltiples patrones
        const companyPatterns = [
            /Denominación\/Razón\s*Social:\s*([A-ZÀ-ÿ\s\d\.,-]+)/i,
            /CONCEPTO\s+RISOGRAFICO\s+SA\s+DE\s+CV/i,  // Patrón específico
            /([A-ZÀ-ÿ\s\d\.,-]+)\s+SA\s+DE\s+CV/i,     // Cualquier nombre + SA DE CV
        ];
        ...
        for (const pattern of companyPatterns) {
            ...
            if (match) {
                data.companyName = match[1] ? match[1].trim() : match[0].trim();
                console.log(`Nombre de empresa extraído: "${data.companyName}"`);
                break;
            }
        }
    ...
    processBancarioDocument(text) {
        ...
        // Extract company name (multiple possible patterns for bank statements)
        const companyPatterns = [
            /([A-ZÀ-ÿ\s\d\.,-]+)\s+SA\s+DE\s+CV/i,
            /([A-ZÀ-ÿ\s\d\.,-]+)\s+S\.?\s*A\.?\s+DE\s+C\.?\s*V\.?/i,
            /Titular:\s*([A-ZÀ-ÿ\s\d\.,-]+)/i,
            /CONCEPTO\s+RISOGRAFICO\s+SA\s+DE\s+CV/i
        ];
    ...
    validateCompanyNames() {
        ...
        // La constancia fiscal SOLO tiene el nombre base (sin SA DE CV, etc.)
        // Los otros documentos tienen el nombre completo con denominación
        
        // Extraer el nombre base de los documentos con denominación completa
        const extractBaseName = (fullName) => {
            return fullName.toUpperCase()
                .replace(/\s+/g, ' ')
                .replace(/\bS\.?A\.?\s*(DE\s*)?C\.?V\.?\b/gi, '') // Quitar SA DE CV y variaciones
                .replace(/\bS\.?C\.?\s*/gi, '') // Quitar SC
                .replace(/\bS\.?R\.?L\.?\s*/gi, '') // Quitar SRL
                .replace(/\bA\.?C\.?\s*/gi, '') // Quitar AC
                .replace(/[.,]/g, '')
                .trim();
        };

        const baseNameOpinion = extractBaseName(opinion.companyName);
        const baseNameConstancia = constancia.companyName.toUpperCase().replace(/\s+/g, ' ').trim();
        const baseNameBancario = extractBaseName(bancario.companyName);

        console.log('Nombres base extraídos:');
        console.log('Opinión:', baseNameOpinion);
        console.log('Constancia:', baseNameConstancia);
        console.log('Bancario:', baseNameBancario);

        // Verificar que el nombre de la constancia esté contenido en los otros dos
        const constanciaInOpinion = baseNameOpinion.includes(baseNameConstancia) || 
                                   baseNameConstancia.includes(baseNameOpinion);
        const constanciaInBancario = baseNameBancario.includes(baseNameConstancia) || 
                                    baseNameConstancia.includes(baseNameBancario);

        console.log('¿Constancia coincide con Opinión?', constanciaInOpinion);
        console.log('¿Constancia coincide con Bancario?', constanciaInBancario);

        if (constanciaInOpinion && constanciaInBancario) {
            console.log('✅ Validación de nombres EXITOSA');
            return true;
        }

        // Fallback: calcular similitud si la coincidencia directa falla
        const similarity1 = this.calculateStringSimilarity(baseNameOpinion, baseNameConstancia);
        const similarity2 = this.calculateStringSimilarity(baseNameBancario, baseNameConstancia);
        
        console.log('Similaridades como fallback:', similarity1, similarity2);
        
        const result = similarity1 > 0.7 && similarity2 > 0.7;
        console.log(result ? '✅ Validación por similitud EXITOSA' : '❌ Validación FALLIDA');
