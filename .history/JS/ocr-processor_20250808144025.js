...
class OCRProcessor {
    ...
    processBancarioDocument(text) {
        ...
        // Extract account number/CLABE - múltiples patrones
        const accountPatterns = [
            /No\.?\s*Cuenta\s+CLABE\s+([0-9]{18})/i,                    // No.Cuenta CLABE 012180004431050153
            /CLABE\s+Interbancaria\s+([0-9]{18})/i,                     // CLABE Interbancaria 002180701848718218
            /Cuenta\s+tradicional\s+([0-9\-]+)/i,                      // Cuenta tradicional 65-51087600-01
            /CLABE\s*:?\s*([0-9]{18})/i,                               // CLABE: 012180004431050153
            /No\.?\s*Cuenta\s*:?\s*([0-9\-]{10,})/i,                   // No. Cuenta: 65-51087600-01
        ];
        ...
        for (const pattern of accountPatterns) {
            ...
            if (match) {
                ...
                console.log(`Bancario - Número de cuenta/CLABE extraído: "${data.accountNumber}"`);
                break;
            }
        }
    ...
    getCompanyInfo() {
        ...
        const bancario = this.documentData.bancario;
        ...
        return {
            ...
            pais: 'México',
            clabeInterbancaria: bancario?.accountNumber || ''
        };
    }
